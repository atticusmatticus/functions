...
contains
   ...
   subroutine ideal_CL3(rBins,rStepSize,cosThBins,cosTh_min,cosTh_max,phiBins,phi_min,phi_max,radius,offset,T, idealHist)
      use idealCfgCL3
      use idealArray
      implicit none
      integer         :: rBins, cosThBins, phiBins, radius
      real(kind=dp)   :: rStepSize, cosTh_min, cosTh_max, phi_min, phi_max, offset, T
      real(kind=dp)   :: idealHist(7,rBins,cosThBins,phiBins)

      ! calculate initial geometry and initialize arrays
      call initial_geom(radius,offset)

      ! compute average force integral.
      call compute_ideal_hist(rBins,rStepSize,cosThBins,cosTh_min,cosTh_max,phiBins,phi_min,phi_max,T, idealHist)
      ...
      do ir = 1, rBins
         do ith = 1, cosThBins
            ...
            do iphi = 1, phiBins
               ...
               idealHist2D(1,ir,ith) = idealHist2D(1,ir,ith) + boltz                               ! g
   ...
   subroutine ideal_2D_to_1D(idealHist2D,rBins,cosThBins, idealHist1D)
      ...
      do ir = 1, rBins
         ...
         do ith = 1, cosThBins
            ...
            idealHist1D(1,ir) = idealHist1D(1,ir) + boltz                           ! g
   ...
   subroutine initial_geom( radius, offset )
      use constants
      use functions
      use idealCfgCL3
      use idealArray
      ...
      psi = pi/real(3,dp)
      do i = 3, nAtoms
         pos_i(i,:) = [0_dp, -1_dp, 0_dp]
         pos_i(i,:) = rotate_x(aEq_HCl, pos_i(i,:))
         pos_i(i,:) = rotate_y(psi, pos_i(i,:))
      ...
      ! Shift the whole molecule by offset in the rCH dimension which is +y
      do i = 1, nAtoms
         pos_i(i,2) = pos_i(i,2) - offset ! offset from C toward the Cl's
      end do
   ...
   subroutine compute_ideal_hist(rBins,rStepSize,cosThBins,cosTh_min,cosTh_max,phiBins,phi_min,phi_max, T, idealHist)
      use constants
      use functions
      use idealCfgCL3
      use idealArray
      ...
      real(kind=dp)               :: rStepSize, cosTh_min, cosTh_max, phi_min, phi_max, potent_en, T, &
         & idealHist(7,rBins,cosThBins,phiBins)

      ! Distance Axis
      allocate( rAxis(rBins), theta(cosThBins), phi(phiBins) )
      rAxis = 0_dp; rMin = 0_dp
      ...
      do ith = 1, cosThBins
         do iphi = 1, phiBins
            ...
            do ir = 1, rBins
               ...
               pos_t = pos_r
               do i = 1, nAtoms
                  pos_t(i,2) = pos_r(i,2) + rAxis(ir) ! translate each solvent atom along y
               end do
               ...
               idealHist(1,ir,ith,iphi) = potent_en/kb_kcalmolK/T             ! -ln(g(r)) == u_dir(r)/kT
   ...
   function lj_energy(i, pos)
      ...
      integer,intent(in)         :: i                    ! inputs not to be changed
      real(kind=dp),intent(in)   :: pos(3)               ! inputs not to be changed
      real(kind=dp)              :: posDist6, lj_energy  ! output

      posDist6 = (pos(1)**2 + pos(2)**2 + pos(3)**2)**(-3)
